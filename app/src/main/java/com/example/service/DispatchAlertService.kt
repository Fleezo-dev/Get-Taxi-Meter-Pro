...
object DispatchAlertController {
    ...
    fun stopAlert(context: Context) {
        try {
            ...
            context.startService(intent)
            context.stopService(Intent(context, DispatchAlertService::class.java))
        } catch (e: Exception) {
            Log.e("DispatchAlertController", "Error stopping alert service: ${e.message}")
        }
    }
}
...
class DispatchAlertService : Service() {
    ...
    private fun startForegroundAlert(
        ...
        // Stop any currently playing audio or vibration before starting a new alert
        stopAudioAndVibration()
        ...
        try {
            ...
            currentRingtone = android.media.RingtoneManager.getRingtone(applicationContext, ringtoneUri)?.apply {
